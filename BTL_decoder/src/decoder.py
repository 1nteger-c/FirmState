...
# Copyright (c) 2025, Suhwan Jeong (https://github.com/1nteger-c)
...
#
# Extended and modified by Suhwan Jeong in 2025 to support additional
# decoding features and memory layout analysis.
...
BTL_SUPPORTED_VERSIONS = [b"1100", b"1200", b'1300', b'1410']
OFFSET1 = {
    b"1100" : 0xc,
    b"1200" : 0x14,
    b'1300': 0x14,
...
def process_btl(data):
    ...
    if BTL_VERSION not in BTL_SUPPORTED_VERSIONS:
        raise BadFileError("Unsupported BTL version '%s' (expected '%s')" % (BTL_VERSION, str(BTL_SUPPORTED_VERSIONS)))

    
    offset += OFFSET1[BTL_VERSION]
...
def process_slog_compressed(data):
    ...
    dst_buf = bytearray()
    lz4_decompress_sequences(memoryview(data)[OFFSET3[BTL_VERSION]:OFFSET3[BTL_VERSION]+compressed_size], dst_buf)
    ...
    return dst_buf, compressed_size+OFFSET3[BTL_VERSION]
...
def process_bufn(data):
    ...
    try:
        process_slog(data[OFFSET2[BTL_VERSION]:OFFSET2[BTL_VERSION]+buf_len])
